...
def main():
    ...
    # Create file list for array job (one filename per line, 0-indexed via bash array)
    ...
    # Throttle concurrency for large arrays (HIVE best practice: >50 tasks need a cap)
    concurrency = min(MAX_CONCURRENT, n)
    array_spec = f"0-{n - 1}%{concurrency}"
    ...
    slurm_script_content = f"""#!/bin/bash --norc
...
# Build 0-indexed file list from the text file
mapfile -t JSON_FILES < "{json_list_file}"
...
# Derive a filesystem-safe output subdirectory name
